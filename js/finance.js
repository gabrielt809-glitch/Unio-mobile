...
function renderFinanceTxForm(){
  ...
  return `<div class="fin-card fin-form-card">
    ...
    <div class="fin-form-grid">
      ...
      <div class="m-grp"><label class="m-lbl">Data</label><input class="field" id="finTxDate" type="date" value="${new Date().toISOString().slice(0,10)}"></div>
...
function financeTxItem(t){
  ...
  return `<div class="fin-tx ${cls}"><div><strong>${unioEscape(t.title)}</strong><span>${new Date(t.date+'T00:00:00').toLocaleDateString('pt-BR')} · ${unioEscape(t.category||'Outros')} · ${unioEscape(meta)}</span></div><div class="fin-tx-side"><b>${sign} ${financeMoney(t.amount)}</b><button onclick="deleteFinanceTx(${t.id})">×</button></div></div>`;
}
