...
function renderCurrentTab(){if(S.curTab==='sleep')renderSleepChart();if(S.curTab==='tasks'){renderWeekStrip();renderTasks();}if(S.curTab==='health'){renderHealth();showActList();}if(S.curTab==='home')renderHome();}
...
['goalModal','customWater','actModal','habModal','settingsModal'].forEach(id=>{document.getElementById(id).addEventListener('click',function(e){if(e.target===this)closeModal(id);});});
