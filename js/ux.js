...
function showBadge(title,sub,ico='🏆'){const b=document.createElement('div');b.className='badge-pop';b.innerHTML=`<div class="badge-ico">${unioEscape(ico)}</div><div class="badge-txt"><div class="badge-title">${unioEscape(title)}</div><div class="badge-sub">${unioEscape(sub)}</div></div>`;document.body.appendChild(b);setTimeout(()=>{b.classList.add('out');setTimeout(()=>b.remove(),280);},3200);}
